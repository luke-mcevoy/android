...
public class ChatService extends Service implements IChatService {
    ...
    private final class SendHandler extends Handler {
        ...
        public void handleMessage(android.os.Message message) {
            ...
            try {
                ...
                String username = data.getString(CHAT_NAME);
                messageText = data.getString(CHAT_MESSAGE);
                Log.i(TAG, "Message Text is: " + messageText);
                receiver = data.getParcelable(RECEIVER);

                sendData = (username + ":" + messageText).getBytes();
                // End todo

                Log.i(TAG, "Sending packet: " + messageText);
    ...
    //            message.timestamp = new Date(Long.parseLong(msgContents[1]));
                    ...
                    Cursor cursor = contentResolver.query(PeerContract.CONTENT_URI, null, selection, selectionArgs, null);
                    if (cursor != null && cursor.getCount() > 0) {
                        cursor.moveToFirst();
                        Peer peer = new Peer(cursor);
                        sender.id = peer.id;
                        sender.writeToProvider(contentValues);
                        contentResolver.update(PeerContract.CONTENT_URI(sender.id), contentValues, null, null);
                    } else {
                        sender.writeToProvider(contentValues);
