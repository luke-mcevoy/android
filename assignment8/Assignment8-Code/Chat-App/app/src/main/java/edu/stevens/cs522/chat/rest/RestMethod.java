...
public class RestMethod {
    ...
    public Response perform(RegisterRequest request) {
        URL url = registerURL(request.chatServer, request.chatname);
        if (url == null) {
    ...
    private URL registerURL(Uri serverUri, String chatName) {

        if (registerUri == null) {
            registerUri = fromURI(serverUri + "?chat-name=" + chatName);
        }
        return registerUri;
    }
    ...
    private URL postMessageURL() {
        ...
        if (postMessageUri == null) {
            ...
            String uri = String.format("%s/%d/messages", serverUri, senderId);
